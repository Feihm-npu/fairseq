...
#         logger.info("overflow_expert1", overflow_expert1)
#         hot_experts = (overflow_expert1 > 0) | (overflow_expert2 > 0)
#         logger.info("hot_experts", hot_experts)
#         num_experts = hot_experts.shape[0]

#         # Pre-process the data to lower precision for hot experts
#         preprocessed_chunks = []
#         for expert_index, chunk in enumerate(dispatched_input.chunk(self.num_local_experts, dim=1)):
#             if hot_experts[expert_index]:
#                 preprocessed_chunks.append(self.lower_precision_tensor(chunk))  # Lower precision
#             else:
#                 preprocessed_chunks.append(chunk)  # Original precision

#         # Perform AllToAll communication
#         all_to_all_chunks = []
#         for chunk in preprocessed_chunks:
#             all_to_all_chunks.append(self.all_to_all_wrapper(chunk))

#         # Process the chunks with experts
#         for expert_index, chunk in enumerate(all_to_all_chunks):
#             expert = self.experts[expert_index % self.num_local_experts]
#             if hot_experts[expert_index]:
#                 chunk = self.restore_precision_tensor(chunk)
#             output = expert(chunk)
#             if hot_experts[expert_index]:
#                 all_expert_outputs.append(self.lower_precision_tensor(output))
#             else:
#                 all_expert_outputs.append(output)

#         logger.info("input dispatched")

#         if self.all2all_size > 1:
#             processed_outputs = []
#             for output in all_expert_outputs:
#                 if isinstance(output, torch.Tensor) and output.dtype == torch.float16:
#                     processed_outputs.append(self.restore_precision_tensor(self.all_to_all_wrapper(output)))
#                 else:
#                     processed_outputs.append(self.all_to_all_wrapper(output))
#             expert_output = torch.cat(processed_outputs, dim=1)
#         else:
#             expert_output = torch.cat(all_expert_outputs, dim=1)

#         logger.info("Output dispatched")
#         expert_output = expert_output.reshape(self.all2all_size * self.num_local_experts, -1, d_model)
...
#         return combined_output, l_aux
    ...
    def quantize_dequantize_fp16_int4(self, tensor_fp16):
        # Ensure the tensor is of type fp16
        assert tensor_fp16.dtype == torch.float16, "Input tensor must be of type fp16"

        # Step 1: Normalize the fp16 values to fit into the range of int4
        min_val = tensor_fp16.min()
        max_val = tensor_fp16.max()
        ...
        # Step 2: Quantize the normalized values into int4
        # int4 range is -8 to 7, so we map the normalized values to this range
        tensor_int4 = (normalized_fp16 * 15 - 8).round().clamp(-8, 7).to(torch.int8)

        # Step 3: Dequantize the int4 values back to fp16
        dequantized_fp16 = (tensor_int4.float() + 8) / 15 * scale + min_val

        return dequantized_fp16.half()

    def quantize_tensor(self, tensor: Tensor) -> Tensor:
    ...
    def dequantize_tensor(self, tensor: Tensor) -> Tensor:
        return tensor.dequantize().to(torch.float16)

    def prepare_for_inference_(self):
        self.in_generation = True

    def all_to_all_wrapper(self, input: Tensor, quantized=False):
