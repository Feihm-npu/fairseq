...
#     metadata["number of expert"] = gates.shape[0]
...
#     print(mask1.shape)
#     print("mask1",mask1)
#     # print("Location",locations1)
#     # overflow_expert1 = torch.sum(mask1 * torch.ge(locations1, capacity))
#     # overflow_expert2 = torch.sum(mask2 * torch.ge(locations2, capacity))
    
#     metadata["overflow_expert1"] = 100 * torch.sum(mask1 * torch.ge(locations1, capacity),dim=0) / torch.sum(mask1)
#     metadata["overflow_expert2"] = 100 * torch.sum(mask2 * torch.ge(locations2, capacity),dim=0) / torch.sum(mask2)

#     # overflow_expert1 = torch.any(mask1 * torch.ge(locations1, capacity), dim=0).float()
#     # overflow_expert2 = torch.any(mask2 * torch.ge(locations2, capacity), dim=0).float()

#     # metadata["overflow_expert1"] = overflow_expert1
#     # metadata["overflow_expert2"] = overflow_expert2
#     # print("In gate function overflow_expert1",metadata["overflow_expert1"])
...
def top2gating(
    ...
    overflow_expert1 = torch.sum(mask1 * torch.ge(locations1, real_capacity), dim=0)
    overflow_expert2 = torch.sum(mask2 * torch.ge(locations2, real_capacity), dim=0)
